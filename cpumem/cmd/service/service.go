...
var fs = flag.NewFlagSet("cpumem", flag.ExitOnError)
var debugAddr = fs.String("debug-addr", ":9080", "Debug and metrics listen address")
var httpAddr = fs.String("http-addr", ":9081", "HTTP listen address")
var grpcAddr = fs.String("grpc-addr", ":9082", "gRPC listen address")
var thriftAddr = fs.String("thrift-addr", ":9083", "Thrift listen address")
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		endpoint, err := zipkingo.NewEndpoint("cpumem", "localhost:80")
		if err != nil {
			logger.Log("err", err)
			os.Exit(1)
		}
