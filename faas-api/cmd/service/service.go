...
var fs = flag.NewFlagSet("faasapi", flag.ExitOnError)
var debugAddr = fs.String("debug-addr", ":10080", "Debug and metrics listen address")
var httpAddr = fs.String("http-addr", ":10081", "HTTP listen address")
var grpcAddr = fs.String("grpc-addr", ":10082", "gRPC listen address")
var thriftAddr = fs.String("thrift-addr", ":10083", "Thrift listen address")
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		endpoint, err := zipkingo.NewEndpoint("faasapi", "localhost:80")
		if err != nil {
			logger.Log("err", err)
			os.Exit(1)
		}
